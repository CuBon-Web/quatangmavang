          ...
          <div class="card">
            <div class="card-body">
              ...
              <seo-assistant
                ...
                @field-manual="markSeoFieldManual"
...
export default {
  ...
  watch: {
    "objData.name"(value) {
      ...
      this.withSyncPause(() => {
        if (!this.seoSyncManual.seoTitle) {
    ...
    "objData.description.0.content"(value) {
      if (this.syncPaused || this.seoSyncManual.metaDescription) return;
      this.withSyncPause(() => {
  ...
  methods: {
    ...
    markSeoFieldManual(field) {
      if (field === "seoTitle") {
        this.seoSyncManual.seoTitle = true;
      }
      if (field === "metaDescription") {
        this.seoSyncManual.metaDescription = true;
      }
    },
    autoOptimizeSeo() {
    ...
    initializeVariantStateFromProduct() {
      const variantSource = Array.isArray(this.objData.variant) ? this.objData.variant : [];
      ...
      this.variantstatus = true;
      this.variantOptions = variantSource.map((option) => ({
        id: this.generateVariantId(),
      ...
      const productCombinations = Array.isArray(this.objData.lungtung) ? this.objData.lungtung : [];
      this.rebuildVariantCombinations();
      if (productCombinations.length > 0) {
        const oldMap = {};
        productCombinations.forEach((item) => {
          oldMap[item.version] = item;
        });
        this.objData.lungtung = this.objData.lungtung.map((item) => ({
          ...item,
          price: Number(oldMap[item.version] ? oldMap[item.version].price : item.price || 0),
          qty: Number(oldMap[item.version] ? oldMap[item.version].qty : item.qty || 0),
          sku: oldMap[item.version] ? oldMap[item.version].sku || "" : item.sku || "",
        }));
      }
    },
    ...
    editById() {
      this.loadings(true);
      Promise.all([
        this.editId({ id: this.$route.params.id }),
        this.listVariantSku({ id: this.$route.params.id })
      ])
        .then(([productResponse, skuResponse]) => {
          this.loadings(false);
          this.objData = productResponse.data;
          this.objData.images = JSON.parse(productResponse.data.images);
          this.objData.preserve = JSON.parse(productResponse.data.preserve);
          this.objData.content = JSON.parse(productResponse.data.content);
          this.objData.description = JSON.parse(productResponse.data.description);
          this.objData.tags = JSON.parse(productResponse.data.tags);
          this.objData.variant = productResponse.data.variant ? JSON.parse(productResponse.data.variant) : [];
          this.objData.lungtung = Array.isArray(skuResponse.data) ? skuResponse.data : [];
          if (productResponse.data.size == "") {
            this.objData.size = [{ title: "", detail: "" }];
          } else {
            this.objData.size = JSON.parse(productResponse.data.size);
          }
          this.withSyncPause(() => {
            this.seoData.seoTitle = productResponse.data.seo_title || this.objData.name || "";
            this.seoData.metaDescription =
              productResponse.data.meta_description || this.objData.description[0].content || "";
            this.seoData.focusKeyword = productResponse.data.focus_keyword || "";
            this.seoData.slug = productResponse.data.slug || this.slugifySeo(this.objData.name || "");
            this.seoSyncManual.seoTitle = !!productResponse.data.seo_title;
            this.seoSyncManual.metaDescription = !!productResponse.data.meta_description;
          });
          this.refreshTagsByCategory();
          this.initializeVariantStateFromProduct();
        })
        .catch(() => {
          this.loadings(false);
          console.log(12);
