...
@endsection
@section('description')
{{ 'Tin tức, bài viết mới nhất về ' . trim($title_page) . ' tại ' . ($setting->webname ?? $setting->company ?? 'website') . '.' }}
@endsection
@section('seo_prev')
{{ $blogPager['prev'] ?? '' }}
@endsection
   ...
   <div class="container">
       <nav aria-label="breadcrumb">
           <ol class="breadcrumb">
               ...
               <li class="breadcrumb-item active" aria-current="page">Tin tức cập nhật</li>
           </ol>
       </nav>
   </div>
</div>
...
<div class="contentWarp ">
   ...
   <section class="blog-layout" itemscope="" itemtype="http://schema.org/Blog">
                                 ...
                                 <div class="custom-article-item_info">
                                    <div class="tags d-flex list-unstyled mb-1">
                                       @php
                                         ...
                                         $itemTags = is_array($itemTags) ? array_slice($itemTags, 0, 3) : [];
                                       @endphp
                                       @foreach ($itemTags as $tag)
                                       <a class="mr-2" href="{{ route('listBlogTag', ['tag' => urlencode($tag)]) }}">#{{ $tag }}</a>
                                       @endforeach
                                    </div>
      ...
      @if(isset($blog) && method_exists($blog, 'links'))
