	...
	try{
		if(!isload){
			...
			$(document).ready(function(){
				// Header search autocomplete: see /frontend/js/search-header.js
						...
						async function goawaySearchcom() {
							...
							setTimeout(function(){
								...
								if(sizeDatacom > 0) {
									...
									if (typeof window.refreshCompareMarks === 'function') {
										window.refreshCompareMarks();
									}
								} else {
			...
			if (wDWs < 991) {
				$('.menu-bar').on('click', function(){
					...
					$('.header-menu > .header-nav').addClass('current');
				})
			...
			$(document).on('click', '#btn-menu-mobile-form, .menu-bar2 > a', function(e){
				e.preventDefault();
				e.stopPropagation();
				$('.opacity_menu').addClass('current');
				$('.header-menu > .header-nav').addClass('current');
...
// So sánh sản phẩm: dùng /frontend/js/compare.js (Laravel session)
// beanComprare.Compare.init();
// $(document).ready(function(){
// 	beanComprare.Compare.compareProduct();
// });

/* Mobile bottom "Danh muc" — open slide menu immediately (no wait for lazy init) */
(function () {
  function openMobileNav(e) {
    var t = e.target;
    if (!t || !t.closest) return;
    var btn = t.closest("#btn-menu-mobile-form, .menu-bar2 > a");
    if (!btn) return;
    e.preventDefault();
    var opacity = document.querySelector(".opacity_menu");
    var nav = document.querySelector(".header-menu > .header-nav, .header-nav-mobile");
    if (opacity) opacity.classList.add("current");
    if (nav) nav.classList.add("current");
  }
  document.addEventListener("click", openMobileNav, true);
})();
